				...
				<c:if test="${!empty member}" >
					<ul class="nav navbar-nav navbar-right">  
						...
						<c:if test="${empty member.memProfile}">
							<li><img src="${contextPath}/resources/images/person.png" class="img-circle" style="width:50px; height:50px;"/> ${member.memName}님, Welcome</li>
						</c:if>	
						<!-- 등록한 이미지 -->
						<c:if test="${!empty member.memProfile}">
							<li><img src="${contextPath}/resources/upload/${member.memProfile}" class="img-circle" style="width:50px; height:50px;"/> ${member.memName}님, Welcome</li>
						</c:if> 	      	
					</ul>
				</c:if>
		    </div>
